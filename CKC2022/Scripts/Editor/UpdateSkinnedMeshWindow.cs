...
public class UpdateSkinnedMeshWindow : EditorWindow
{
    ...
    private GUIContent statusContent = new("Waiting...");
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Update Skinned Mesh Renderer"))
        {
            ...
            // Look for root bone
            string rootName = "";
            if (targetSkin.rootBone != null)
                rootName = targetSkin.rootBone.name;

            Transform newRoot = null;

            // Reassign new bones
            var newBones = new Transform[targetSkin.bones.Length];
            var existingBones = rootBone.GetComponentsInChildren<Transform>(includeInactive);
            int missingBones = 0;
